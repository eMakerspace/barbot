...
void DisplayController::setWorking() {
    mode_      = Mode::Working;
    modeStart_ = millis();
}

void DisplayController::setEStop() {
    mode_      = Mode::EStop;
    modeStart_ = millis();
}
...
void DisplayController::tick() {
    switch (mode_) {
        ...
        case Mode::Working:    updateWorking();    break;
...
// Emergency stop: both digits show "E" (a+f+g+e+d), flashing 300ms on / 300ms off
void DisplayController::updateEStop() {
    // "E" glyph: top bar (a) + left verticals (f, e) + middle bar (g) + bottom bar (d)
    constexpr uint8_t E_GLYPH = SEG_A | SEG_F | SEG_G | SEG_E | SEG_D;
    uint32_t t  = millis() - modeStart_;
    uint8_t  br = (t % 600 < 300) ? 255 : 0;
    show(E_GLYPH, E_GLYPH, br, br);
}

// Work-in-progress animation: spinner on ones, breathing on tens
void DisplayController::updateWorking() {
    uint32_t t = millis() - modeStart_;

    // Ones: clockwise spinner (a→b→c→d→e→f)
    // 120 ms per step = 720 ms per full cycle
    int step = (t / 120) % 6;
    uint8_t onesMask = RING[step];
    driver_.setRawMaskOnes(onesMask);
    driver_.setBrightnessOnes(255);

    // Tens: breathing middle bar (g segment)
    uint8_t tensBr = sinBright(t, 2000, 80, 255);
    driver_.setRawMaskTens(SEG_G);
    driver_.setBrightnessTens(tensBr);
}
